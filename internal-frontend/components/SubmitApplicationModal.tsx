...
export interface ApplicationFormData {
  phoneNumber: string;
  applicationType: ("Professional Practice" | "Individual Growth")[];
  confirmed: boolean;
}
...
export default function SubmitApplicationModal({
  ...
  const canSubmit = confirmed && selectedTypes.length > 0;

  const handleSubmit = () => {
    if (!canSubmit) return;
    onSubmit({ phoneNumber, applicationType: selectedTypes, confirmed });
  };
